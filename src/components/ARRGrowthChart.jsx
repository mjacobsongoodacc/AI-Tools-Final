  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-bone-40 text-[10px] font-semibold uppercase tracking-wide mb-1">Current ARR</p>
          <p className="font-mono-data text-2xl font-bold text-white">{formatArr(latest?.arr)}</p>
        </div>
        <div className="text-right">
          <p className="text-bone-40 text-[10px] font-semibold uppercase tracking-wide mb-1">QoQ Growth</p>
          <div className={`inline-flex items-center gap-1 font-mono-data text-lg font-bold ${up ? 'text-green-400' : 'text-accent'}`}>
            {qoq != null && Number.isFinite(qoq) ? (
              ...
              <span className="text-bone-40 font-medium text-base">—</span>
            )}
          </div>
        </div>
      </div>
